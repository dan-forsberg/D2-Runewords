...
function scrape() {
    ...
    async function fetchRuneWordData(runeword) {
        let runewordURI = runeword.replace(" ", "_");
        ...
        if (res.ok) {
            ...
            //@ts-expect-error shush
            const item = document.querySelector("div[class='pi-data-value pi-font']").innerText;

            return { runeword: runeword, runes: runes, effects: magic, item: item };
        } else {
